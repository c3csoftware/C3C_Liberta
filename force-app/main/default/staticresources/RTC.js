...
const WebphoneL5 = {
    init: ({ extension, password, server, hideInterface = false, showCdr = false, iframeId = "widgetIframeWebphoneL5", btnId = "btnIframeWebphoneL5" }) => {
        ...
        if (!started) {
            ...
            includeIframe({ iframeId: initParams.iframeId, btnId: initParams.btnId })
            ...
            serverAddress = protocol + server.replace('http://', '').replace('https://', '');
...
const includeIframe = ({ iframeId, btnId }) => {
    ...
    document.querySelector("body").append(btnWidget, bodyWidget)
}
